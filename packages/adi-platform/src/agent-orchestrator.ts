...
export function createAgentOrchestrator(deps: AgentOrchestratorDeps): AgentOrchestrator {
  const { eventBus, registry, connectorManager, fetchEnabled = false } = deps;
  ...
  return {
    async onPipelineStarted(context, correlationId) {
      ...
      for (const providerId of providerIds) {
        const providerPackages = packages.filter((pkg) => pkg.sourceId.startsWith(providerId));
        await eventBus.publish("adi.evidence.received", {
          ...
          degraded: providerPackages.length === 0,
        });
      }
    },
